public class Wordle {
    ...
    public static String[] readDictionary(String filename) {
        In rdr = new In(filename);
        return rdr.readAllStrings();
    }
    ...
    public static String chooseSecretWord(String[] dict) {
        int v1 = (int)(Math.random() * dict.length);
        return dict[v1];
    }
    ...
    public static boolean containsChar(String secret, char c) {
        for (int v2 = 0; v2 < secret.length(); v2++) {
            if (secret.charAt(v2) == c) return true;
        }
        return false;
    }
    ...
    public static void computeFeedback(String secret, String guess, char[] resultRow) {
        for (int p1 = 0; p1 < secret.length(); p1++) {
            if (guess.charAt(p1) == secret.charAt(p1)) {
                resultRow[p1] = 'G';
            } else if (containsChar(secret, guess.charAt(p1))) {
                resultRow[p1] = 'Y';
            } else {
                resultRow[p1] = '_';
            }
    ...
    public static void storeGuess(String guess, char[][] guesses, int row) {
        for (int p2 = 0; p2 < guess.length(); p2++) {
            guesses[row][p2] = guess.charAt(p2);
        }
    }
    ...
    public static boolean isAllGreen(char[] resultRow) {
        for (int g1 = 0; g1 < resultRow.length; g1++) {
            if (resultRow[g1] != 'G') return false;
        }
        return true;
    }
